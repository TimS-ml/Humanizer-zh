...
### 🔤 语言和语法模式（7种）
7. 过度使用的"AI 词汇"（中文原生高频词 + 英文直译腔）
8. 中文句法四规则（一句一判断、术语先白话、被/把预算、连接词预算）
9. 避免使用"是"（系动词回避）
10. 否定式排比
11. 三段式法则过度使用
12. 刻意换词（同义词循环）
...
此外还有三条**硬约束**，优先级高于所有模式：不碰标点（交给外部 formatter）、英文技术词不强行汉化（用 "Token" 不用 "词元"）、保留口语化技术风格。
...
1. **识别 AI 模式** - 对照 `SKILL.md` 中列出的 23 种模式扫描文本
...
- 面向技术写作做了本地化：英文术语不强行汉化、保留口语化技术风格
