...
“审阅”“诊断”“列问题”会进入审阅模式。“改写”“润色”会进入改写模式。未指定时默认改写。
...
**输出示例：**
- 高：`信息来源`和`两种方法`中的“硬顶”“泵”“搬运”没有白话定义或机制对应，私人隐喻直接承担了解释。修正时先说明实际机制，再决定是否保留标签。
- 中：目标提出“怎样取得新的任务信息”，后续章节只给标签，没有说明信息怎样进入系统。需要补出从信息来源到两种方法的过渡。
- 低：`DPI`是标准缩写，不因英文形式报错。若目标读者可能不熟悉，可补全称和一句白话解释。

审阅模式到问题清单为止，不输出重写后的全文。

## 检测的 AI 写作模式

完整技术笔记、文章和多章节文档会先运行四项无编号深层检查：
...
区分来源结果、来源作者解释、笔记作者推断、工作假说、预测和来源待补，并配对前文结论与后置 Caveat。
...
- 完整技术文档会区分标准缩写与私人标签：只在目标读者可能不懂时展开缩写；已经定义且稳定指代的“外部锚点”等本地标签可以保留
- 公式使用完整变量、作用域和假设时不报错；形式化措辞超过推导或测量支持时才命中
