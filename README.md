...
- 中：当前语境不足以判断 `DPI` 的全称，不能因为它是大写缩写就直接保留或猜测。应要求作者补充；只有信息论语境明确且它指 Data Processing Inequality 时，才作为标准缩写保留。目标读者可能不熟悉时，再补全称和一句白话解释。
...
区分来源结果、来源作者解释、笔记作者推断、工作假说、预测和来源待补，并配对前文结论与后置限制条件 (Caveat)。忠实转述有引用的论文结果时，不要求在笔记里重复完整测量方法；精确结果仍须可追溯。
...
- 完整技术文档会根据领域语境区分标准缩写与私人标签，不从大写形式猜全称；已经定义且稳定指代的“外部锚点”等本地标签可以保留
- 公式使用完整变量、作用域和假设时不报错；有引用的论文结果只需可追溯，作者新造指标或扩大结论时才检查操作化定义和测量
